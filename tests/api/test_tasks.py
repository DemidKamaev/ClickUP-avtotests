...
class TestApi:
    ...
    @allure.title("Создание и удаление задачи без использование фикстур")
    @allure.description(
        "Create task, check fields task, Delete task, check delete task"
    )
    def test_create_task(self, create_data, clickup_client, get_list_id_fixture):
        with allure.step("Create new task"):
            list_id = get_list_id_fixture
            create_response = clickup_client.create_task(list_id, create_data)
        ...
        with allure.step("Проверка наличия ID в ответе"):
            assert 'id' in task_data, "Отсутствует ID по ключу 'id' в ответе"
            assert isinstance(task_data['id'], str), (f"Значение по ключу 'id' имеет другой тип данных: "
                                                      f"{type(task_data['id'])}")
        ...
        if 'status' in task_data:
            with allure.step("Проверка полей во вложенном словаре status"):
                assert isinstance(task_data['status'], dict), (f"Значение по ключу 'status' имеет другой тип данных: "
                                                               f"{type(task_data['status'])}")
                assert isinstance(task_data['status']['id'], str), (f"Значение по ключу 'id' имеет другой тип данных: "
                                                                    f"{type(task_data['status']['id'])}")
                assert len(task_data['status']['id']) > 0, "Отсутствует значение по ключу 'id' в dict['status']"

                assert isinstance(task_data['status']['status'], str), (f"Значение по ключу 'status' имеет другой тип "
                                                                        f"данных: {type(task_data['status']['status'])}")
                assert task_data['status']['status'] == create_data['status'], "Статус задачи не совпадает"

                assert isinstance(task_data['status']['color'], str), (f"Значение по ключу 'color' имеет другой тип "
                                                                       f"данных: {type(task_data['status']['color'])}")
                assert len(task_data['status']['color']) > 0, "Отсутствует значение по ключу 'color' в dict['status']"

                assert isinstance(task_data['status']['orderindex'], int), (f"Значение по ключу 'color' "
                                                                            f"имеет другой тип данных: "
                                                                            f"{type(task_data['status']['orderindex'])}")
                assert task_data['status']['orderindex'] == 0, ("Отсутствует значение по ключу "
                                                                "'orderindex' в dict['status']")

                assert isinstance(task_data['status']['type'], str), (f"Значение по ключу 'type' "
                                                                      f"имеет другой тип данных: "
                                                                      f"{type(task_data['status']['type'])}")
                assert len(task_data['status']['type']) > 0, ("Отсутствует значение по ключу 'type' в dict['status']")

        if 'creator' in task_data:
            with (allure.step("Проверка полей во вложенном словаре creator")):
                assert isinstance(task_data['creator'], dict), (f"Значение по ключу 'creator' имеет другой тип данных:"
                                                                f"{type(task_data['creator'])}")

                assert isinstance(task_data['creator']['id'], int), (f"Значение по ключу 'id' имеет другой тип данных: "
                                                                     f"{type(task_data['creator']['id'])}")

                assert isinstance(task_data['creator']['username'], str), (f"Значение по ключу 'username' имеет другой"
                                                                           f" тип данных: "
                                                                           f"{type(task_data['creator']['username'])}")
                assert len(task_data['creator']['username']) > 0, ("Отсутствует значение по ключу 'username' "
                                                                   "в dict['creator']")

                assert isinstance(task_data['creator']['color'], str), (f"Object not found or object not type dict: "
                                                                        f"{task_data['creator']['color']}")
                assert task_data['creator']['color'] == "", ("Отсутствует значение по ключу 'color' "
                                                             "в dict['creator']")

                profile_picture = task_data['creator']['profilePicture']
                assert isinstance(profile_picture, str) or profile_picture is None, \
                    (f"Если profilePicture не строка и не равен None, поднимется исключение: "
                     f"{task_data['creator']['profilePicture']}")

        with allure.step("Проверка совпадания ID списка"):
            assert isinstance(task_data['list'], dict), f"Тип данных 'list': {type(task_data['list'])}"
            assert isinstance(task_data['list']['id'], str), f"Тип данных 'id': {type(task_data['list']['id'])}"
            assert len(task_data['list']['id']) > 0, f"Отсутствует значение по id: {task_data['list']['id']}"

        with allure.step("Проверка совпадания ID folders"):
            assert isinstance(task_data['folder'], dict), f"Тип данных 'folder': {type(task_data['folder'])}"
            assert isinstance(task_data['folder']['id'], str), f"Тип данных 'id': {type(task_data['folder']['id'])}"
            assert len(task_data['folder']['id']) > 0, f"Отсутствует значение по id: {task_data['folder']['id']}"

        with allure.step("Проверка совпадения ID spaces"):
            assert isinstance(task_data['space'], dict), f"type 'space': {type(task_data['space'])}"
            assert isinstance(task_data['space']['id'], str), f"type 'id': {type(task_data['space']['id'])}"
            assert len(task_data['space']['id']) > 0, f"value key 'id' not found: {task_data['space']['id']}"

    @pytest.fixture(scope='function')
    def test_create_task(self, create_data, get_list_id):
        list_id = get_list_id
